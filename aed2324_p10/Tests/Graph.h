...
vector<T> Graph<T>::dfs() const {
    ...
    // Reset the visited state for future calls to dfs.
    for (auto v : vertexSet) {
        v->setVisited(false);
    }
...
vector<T> Graph<T>::bfs(const T & source) const {
    ...
    if (startVertex == nullptr) {
        return res; // Return an empty vector if the source vertex is not found.
    }
    ...
    // Reset the visited state for future calls to bfs.
    for (auto v : vertexSet) {
        v->setVisited(false);
    }
