...
bool isConnected() {
  ...
  // --------------------------------------------------------------
  int countSCCs() { // Função para contar o numero de componentes fortemente conectados SCCs
    ...
    auto dfs = [&](int u, auto&& dfs) -> void {
      stack.push(u); // Adiciona o nó atual à pilha 
      onStack[u] = true; // Marca o nó como estando na pilha
      ids[u] = low[u] = id++; // Inicializa o ID de descoberta e o valor de low para o nó
    // Explora os vizinhos do nó u
      for (const auto& edge : nodes[u].adj) {
        int v = edge.dest; // Nó de dest
        if (ids[v] == -1) {  // Node v has not been visited
          dfs(v, dfs); // Realiza DFS recursivamente no nó v
          low[u] = std::min(low[u], low[v]); // Atualiza o valor de low de u
        } else if (onStack[v]) {  // Node v is in the current stack
          low[u] = std::min(low[u], ids[v]); // Atualiza low[u] com o menor ID da pilha
        }
      }
      ...
      if (ids[u] == low[u]) {
        while (!stack.empty()) {
          int node = stack.top(); // Remove os nós da pilha
          stack.pop();
          onStack[node] = false;
          low[node] = ids[u]; // Assign the same component
          if (node == u) break; // Para quando encontrar o nó raiz
        }
    ...
    for (int i = 1; i <= n; i++) {
      if (ids[i] == -1) dfs(i, dfs); // Se o nó ainda não foi visitado inicializa o DFS a partir deste nó
    }
    // Retorna o numero total de SCCs encontradas
    return sccCount;
  }
    // Função patra encontrar o caminho mais curto entre os nós a e b num grafo não direcionado
    //Usa BFS para garantir que o menor numero de arestas no caminho
    ...
    // Inicializa a BFS
    q.push(a);
    dist[a] = 0; // Distacia de a para ele mesmo 

    while (!q.empty()) {
        int u = q.front(); // Pega o nó da frente de fila
        q.pop();
        // Explora os vizinhos do nó atual u
        for (const auto& edge : nodes[u].adj) {
            int v = edge.dest;
            ...
            if (dist[v] == -1 || dist[v] > dist[u] + 1 || 
                ...
                dist[v] = dist[u] + 1; // distanca minima
                pred[v] = u; // u é o precessor de v
                q.push(v); // Adicina v à fila para continuar a BFS
            }
        }
    }
    ...
    for (int at = b; at != -1; at = pred[at]) {
        path.push_front(at); // Adiciona os nós ao caminho em ordem reversa
    }

    return path; // Retorna o caminho reconstruido de a para b
}
    ...
    auto dfs = [&](int u, auto&& dfs) -> void {
        stack.push(u); // Adicina o nó à pilha 
        onStack[u] = true; // Maraca o nó como presente na pilhs 
        ids[u] = low[u] = id++; // Inicializa os IDs e menor ID alcançavel
        // Itera sobre os vizinhos do nó u
        for (const auto& edge : nodes[u].adj) {
            int v = edge.dest;
            if (ids[v] == -1) {  // Se `v` ainda não foi visitado
                dfs(v, dfs); // executa o DFS recursivamente
                low[u] = std::min(low[u], low[v]);
            } else if (onStack[v]) {  // Se `v` está na pilha
                low[u] = std::min(low[u], ids[v]); // Atualiza o menor ID alcançavel
            }
        }

        // Identifica um SCC
        // verifica de u é raiz de um Scc
        if (ids[u] == low[u]) {
            std::set<int> scc; // conjunto para armazenar os nós da SCC atual
            while (true) {
                int node = stack.top(); // Remove nós da pilha
                stack.pop();
                onStack[node] = false;
                scc.insert(node); // Adiciona o nó ao SCC atual
                if (node == u) break; // Quando alcançar o nó raiz
            }
            sccs.insert(scc); // Adiciona o SCC ao conjunto SCC
        }
    };
    ...
    return sccs; // retorna todos os SCC comom um conjunto de conjuntos
}
